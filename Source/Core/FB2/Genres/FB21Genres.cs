...
namespace Core.FB2.Genres
{
	...
	public class FB21Genres : IFBGenres
	{
		...
		public string GetFBGenreName( string sGenreCode ) {
			...
			return stringProcessing.OnlyCorrectSymbolsForString( m_dFB21Genres[sGenreCode] );
		}
		...
		public string GetFBGenreGroup( string sGenreCode ) {
			...
			return stringProcessing.OnlyCorrectSymbolsForString( m_dFB21GenresGroup[sGenreCode] );
		}
		
		public string[] GetFBGenreNamesArray() {
			return m_sFB21GenreNames;
		}
		public string[] GetFBGenreCodesArray() {
			return m_sFB21GenreCode;
		}
		
		public List<string> GetFBGenresForGroup( string sGGroup ) {
			if( m_dFB21GenresGroup.ContainsValue( sGGroup ) ) {
				m_lsGenresForGroup = new List<string>();
				foreach( string g in m_dFB21GenresGroup.Keys ) {
					// m_dFB21GenresGroup.Values - Группы; m_dFB21GenresGroup.Keys - Жанры
					if( m_dFB21GenresGroup[g] == sGGroup ) {
